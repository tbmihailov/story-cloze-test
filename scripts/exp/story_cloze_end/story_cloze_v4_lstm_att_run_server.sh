...
data/generated_proc_pos_ROCStories_ROC-16-17-mutate_rocstories_data_pos_v1_20_17-02-20-12-01-35.json.easy5.txt
data/generated_proc_pos_ROCStories_ROC-16-17-mutate_rocstories_data_pos_v1_20_17-02-20-12-01-35.json.easy4.txt
data/generated_proc_pos_ROCStories_ROC-16-17-mutate_rocstories_data_pos_v1_20_17-02-20-12-01-35.json.easy3.txt
data/generated_proc_pos_ROCStories_ROC-16-17-mutate_rocstories_data_pos_v1_20_17-02-20-12-01-35.json.easy2.txt
data/generated_proc_pos_ROCStories_ROC-16-17-mutate_rocstories_data_pos_v1_20_17-02-20-12-01-35.json.easy1.txt
data/generated_proc_pos_ROCStories_ROC-16-17-mutate_rocstories_data_pos_v1_20_17-02-20-12-01-35.json.easy0.txt
...
for ((i=6;i<END;i++)); do
    ...
    for ((run_id_it=0;run_id_it<num_runs;run_id_it++)); do
        for ((run_te=0;run_te<2;run_te++)); do
            for ((run_lstm_out=0;run_lstm_out<2;run_lstm_out++)); do
                for ((run_id=0;run_id<1;run_id++)); do
                    for ((run_hs=0;run_hs<5;run_hs++)); do
                        for ((run_bs=0;run_bs<4;run_bs++)); do
                            run_name=story_cloze_v4_lstm_att_emb${loop_emb_name}
                    ...
                    #        dev_data_submission_name="roc_random_spring2016"
                            ...
                            run_name=${run_name}_tr.${train_data_name}_d.${dev_data_name}_te.${test_data_name}_mlp${use_mlp}

                            echo "=========Settings==========="
                            # NN model settings
                            lstm_hidden_size=${hidden_size_list[$run_hs]}  # 384
                            lstm_layers=1
                            batch_size=${batch_size_list[$run_bs]}  # 50
                            eval_every_steps=0
                            eval_every_epochs=1
                            ...
                            train_embeddings=${train_emb_list[${run_te}]} # False

                            short_embeddings=False
                            attention_repr=True
                            lstm_out_repr=${use_lstm_out_list[$run_lstm_out]}  # False
...
#
#echo "=========Addressing complex and subjective product-related queries with customer reviewsEVAL========="
#cmd=eval
#
