...
from models.story_cloze_memory_network_reader import StoryClozeMemoryNetworkReader
...
def pad_data_and_return_seqlens_dim3(data, story_max_len, pad_value_sent=[0], pad_value_token=0, ):
    '''
    Pad data with 3 dimentsion - batchsize x story sents count x sentence size
    :param data:
    :param pad_value_sent:
    :param pad_value_token:
    :return:
    '''
    # logging.info("data:%s" % (len(data)))
    # story same size as
    batch_data_seqlens_story_sents = np.asarray([len(a) for a in data])
    if story_max_len == 0:
        story_max_len = max(batch_data_seqlens_story_sents)
    # logging.info("story_max_len:%s" % story_max_len)
    batch_data_padded_x_story = [pad(a, pad_value_sent, story_max_len) for a in data]

    # logging.info("batch_data_padded_x_story:%s" % (len(batch_data_padded_x_story)))
    sent_max_size = 0
    batch_data_seqlens_story_sent_tokens = []
    for story in batch_data_padded_x_story:
        story_sents_sizes = []
        for sent in story:
            story_sents_sizes.append(len(sent))

            sent_max_size = max(sent_max_size, len(sent))
        batch_data_seqlens_story_sent_tokens.append(story_sents_sizes)

    #debug logging.info("batch_data_seqlens_story_sent_tokens:%s" % (len(batch_data_seqlens_story_sent_tokens)))

    batch_data_padded_x_sents = []
    for story in batch_data_padded_x_story:
        story_padded = [pad(a, pad_value_token, sent_max_size) for a in story]
        batch_data_padded_x_sents.append(story_padded)

    batch_data_padded_x_sents = np.asarray(batch_data_padded_x_sents)
    return batch_data_padded_x_sents, batch_data_seqlens_story_sent_tokens, batch_data_seqlens_story_sents
...
class StoryCloze_v5_mem_v1_run(object):
    ...
    def train(self,
        ...
        input_list_stories, input_list_endings1, input_list_endings2, input_list_labels, input_list_story_ids = transofrm_data_word_to_id(
            ...
            sentence_wise_story=sentence_wise_story_repr)  # mutate_data means that we generate 2 entries - Ending1, Ending 2 and Ending2, Ending 1
        ...
        if not train_random_batch_size:
            ...
            evaluate_every = options.eval_every_steps if options.eval_every_steps > 0 else options.eval_every_epochs * (
            data_len / batch_size)
        else:
            ...
            train_batches = batch_iter_random_batch_per_steps(data_train_all, batch_sizes=batch_sizes,
        ...
        in_dev_list_stories, in_dev_list_endings1, in_dev_list_endings2, in_dev_list_labels, in_dev_list_story_ids = transofrm_data_word_to_id(
            ...
            lowercase=True,
            tokens_field=tokens_field,
            sentence_wise_story=sentence_wise_story_repr
        ...
        def eval_data(sess, data_dev_all, gold_labels, save_features=False, save_features_file="file.features.pickle"):
            ...
            for batch in dev_batches:
                ...
                batch_stories_padded, batch_stories_seqlen, _ = pad_data_and_return_seqlens_dim3(batch_stories, story_max_len=self._story_max_len, pad_value_sent=[0], pad_value_token=0)
                batch_endings1_padded, batch_endings1_seqlen = pad_data_and_return_seqlens(batch_endings1, pad_value=0)
                batch_endings2_padded, batch_endings2_seqlen = pad_data_and_return_seqlens(batch_endings2, pad_value=0)

                res_cost, res_acc, res_pred_y, res_feats = dnn_model.dev_step(sess,
                                                                              zip(batch_stories_padded,
                                                                                  batch_stories_seqlen,
                                                                                  batch_endings1_padded,
                                                                                  batch_endings1_seqlen,
                                                                                  batch_endings2_padded,
                                                                                  batch_endings2_seqlen
                                                                                  ), batch_labels)
        ...
        with tf.Graph().as_default() as curr_graph:
            ...
            with sess.as_default():
                dnn_model = StoryClozeMemoryNetworkReader(
                    n_classes=n_classes,
                    ...
                    sent_encoder="LSTM",  # "PE", "BoW", "LSTM"
                    memory_size=self._story_max_len
                )
                ...
                for batch in train_batches:
                    ...
                    #debug logging.info("batch_stories:%s" % str(np.asarray(batch_stories).shape))
                    batch_stories_padded, batch_stories_seqlen, _ = pad_data_and_return_seqlens_dim3(batch_stories,
                                                                                                  story_max_len=self._story_max_len,
                                                                                                  pad_value_sent=[0],
                                                                                                  pad_value_token=0)

                    #debug logging.info("batch_stories_padded:%s" % str(np.asarray(batch_stories_padded).shape))
                    batch_endings1_padded, batch_endings1_seqlen = pad_data_and_return_seqlens(batch_endings1, pad_value=0)
                    batch_endings2_padded, batch_endings2_seqlen = pad_data_and_return_seqlens(batch_endings2, pad_value=0)
                    ...
                    if current_step % evaluate_every == 0:
                        ...
                        log_summary_results(summary, "data_train", current_step, curr_eval, curr_epoch_acc_train,
                                            curr_epoch_mean_loss_train)

                        curr_eval += 1
                        logging.info("=== %s steps in %s" % (epoch_steps, ti.time() - epoch_start))
                        logging.info("===== EVAL %s =====" % (curr_eval))

                        def eval_and_log(summary, sum_group_name, data_dev_all, in_dev_list_labels, options,
                                         save_submission, save_min_acc, dev_acc=0.00, dev_acc_best=0.00):
                            logging.info("%s data eval:" % sum_group_name)
                            eval_start = ti.time()
                            # eval dev
                            prec_rec_f_supp, overal_loss, overall_accuracy, predictions_all, ids_all, conf_matrix = eval_data(
                                sess, data_dev_all, in_dev_list_labels, save_features=options.save_features,
                            ...
                            if save_submission and overall_accuracy > prev_best_acc and overall_accuracy >= save_min_acc:
                                ...
                                if options.submission_data_eval:
                                    try:
                                        submission_file_name = options.submission_data_eval + "_%s_st%s_evst%s_devacc_%s_acc_%s.txt" % (
                                        sum_group_name, current_step, curr_eval, dev_acc, overall_accuracy)
                                        ...
                                        submission_file_safe = "%s/%s_submission_dev.txt" % (
                                        self._output_dir, self._run_name)
                        ...
                        # eval data dev
                        dev_acc, dev_acc_prev_best = eval_and_log(summary, "data_dev", data_dev_all, in_dev_list_labels,
                                                                  options,
                                                                  save_submission=options.save_submission_dev == "True",
                                                                  save_min_acc=options.save_min_acc)

                        if data_test_all is not None:
                            # eval data test
                            eval_and_log(summary, "data_test", data_test_all, in_test_list_labels, options,
                                         save_submission=True, save_min_acc=options.save_min_acc, dev_acc=dev_acc,
                                         dev_acc_best=dev_acc_prev_best)
                        # eval dev
...
if __name__ == "__main__":
    ...
    parser.add_option("--save_min_acc", dest="save_min_acc", default=0.00,
                      help="min threshold for exporting submission file", type="float")
    parser.add_option("--save_submission_dev", dest="save_submission_dev", default=False,
                      help="Export dev submission file (for debug)")
    ...
    if options.emb_model_type == "w2v":
        ...
        embeddings_vec_size = embeddings_model.wv.syn0.shape[1] if embeddings_model.wv else embeddings_model.syn0.shape[
            1]
    ...
    magic_box = StoryCloze_v5_mem_v1_run(output_dir=options.output_dir,
